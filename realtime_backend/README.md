...
### 5. 断线重连健壮性 🔁（阶段 6 校验）
- 缓存最近一次 `simulation_init`，新客户端连上即重放，无需等待核心重发
- 仿真核心断开时后端保持运行：客户端连接不受影响，命令返回 `no_core_connected` 错误
- 客户端断开自动清理，广播失败不会拖垮服务
- 仿真核心（`demo_sim_core`）运行中连接断开后按 2 s 退避自动重连并重发 init
- 前端 `websocket.js` 自带指数退避重连——三层各自具备断线自愈能力
- 复现脚本：`hypatia-master/satviz/test_reconnect.py`（R1–R5 全部通过）
