...
children.push(body("商业航天已被写入政府工作报告，工信部《制造业人才发展规划指南》预测航空航天装备领域人才缺口达 47.5 万人，而高校卫星网络教学长期面临“工具重、成本高、看不见、无答案”四大痛点。星桥以“轻量实时 + 可验证 + 开源生态”切入高校虚拟仿真实验教学市场，采用“开源社区免费 + 高校私有化部署订阅 + 虚拟仿真一流课程共建 + 企业培训”的商业模式，规划三年内完成“本校验证—区域推广—全国平台”三步走。"));
...
children.push(makeTable({
  ...
  rows: [
    ...
    ["SOM", "三年可获取", "粤港澳大湾区与长三角先行高校 100~150 所的试点与订阅，及 10~20 项课程共建", "约 800~2500 万元（三年累计）"]
  ]
}));
...
children.push(body("当前正处于三重窗口叠加期：其一，千帆、GW 星座 2025 年起密集组网，“卫星网络”从前沿话题变为产业常识，课程需求爆发在即；其二，国家级一流课程“双万计划”与教育数字化战略推动高校持续投入虚拟仿真资源；其三，现有供给（科研工具与定制项目）结构性错位，尚无占据心智的教学品牌。窗口预计 2~3 年，先完成课程共建卡位者将获得教师群体的迁移成本优势。"));
...
children.push(makeTable({
  ...
  rows: [
    ...
    ["高校订阅版", "高校院系", "私有化部署 + 班级管理 + 实验库 + 自动报告 + 升级支持", "按年订阅，2~8 万元/校/年（按规模分级）"],
    ["课程共建", "一流课程建设团队", "联合定制实验与申报材料，共享课程成果", "按项目 15~50 万元/项"],
    ["企业培训版", "商业航天企业、院所", "定制场景（自有星座构型）+ 集训服务", "按项目报价，8 万元起"]
  ]
}));
...
children.push(makeTable({
  ...
  rows: [
    ...
    ["区域推广", "2027.7 ~ 2028.6", "粤港澳 + 长三角 10~20 所高校试点", "以样板课程 + 共建协议切入；参加教学学术会议；开源社区破千星"],
    ["全国平台", "2028.7 起", "标准化的“实验即服务”，成为细分领域默认选项", "上线 SaaS 版；实验库开放投稿分成；对接国家级虚拟仿真项目体系"]
  ]
}));
children.push(h2("7.2  运营重心"));
children.push(bullet("实验内容持续供给：每学期新增 2~3 个实验（如路由算法对比、雨衰影响、数字孪生实时模式），保持课程新鲜度；"));
...
children.push(makeTable({
  ...
  rows: [
    ...
    ["远期（24~36 个月）", "数字孪生化", "接入实时 TLE 与遥测数据源，从“仿真器”升级为“真实星座数字孪生”教学与科普平台；多校互联的联合实验（分布式组网大实验）"]
  ]
}));
...
children.push(makeTable({
  ...
  rows: [
    ...
    ["2027 Q2", "本校课程落地 1~2 门", "真实授课 ≥ 100 学生人次；课前课后测评显著提升"],
    ["2027 Q4", "试点高校 10 所 + 教改论文 1 篇", "签署共建/试点协议；论文投稿实验类期刊"],
    ["2028 Q4", "订阅学校 30+ 所，收入覆盖团队成本", "年度经常性收入 ≥ 150 万元（中性情景）"],
    ["2029 起", "SaaS 上线 + 实验库生态", "注册教师 500+；社区实验投稿常态化"]
  ]
}));
...
children.push(body("平台边际成本极低是商业模式的根基：单实例状态流约 200 KB/s/学生（5 Hz × <50 KB 帧压缩后典型值），一台 8 核教学服务器即可支撑 1~2 个教学班并发；软件交付无实体成本，主要成本为研发人力（在校团队 + 勤工助学）与内容制作。按订阅价 2~8 万元/校/年、单校支撑成本 < 0.3 万元/年估算，毛率超过 90%。"));
children.push(h2("9.3  收入预测与盈亏平衡"));
...
children.push(makeTable({
  ...
  rows: [
    ["2027", "10~24（3~6 校）", "15~40（1~2 项）", "0~8", "25~72 / 15~40"],
    ["2028", "60~160（20~35 校）", "45~100（3~5 项）", "8~24", "113~284 / 70~150"],
    ["2029", "150~400（50~100 校）", "90~200", "24~60", "264~660 / 160~350"]
  ]
}));
children.push(body("按中性情景，2028 年度收入即可覆盖团队成本与服务器支出实现盈亏平衡；保守情景下平衡点落在 2029 年。敏感性主要来自高校采购周期（6~12 个月），对策是以共建项目（预算走课程建设经费、周期短）先行，订阅制随后跟进。"));
...
children.push(body("近中期，项目通过课程共建与订阅服务带动在校研发、内容制作等岗位；远期随 SaaS 化将形成稳定的技术服务团队。更重要的是间接带动：每一所接入星桥的院校，都在为商业航天产业输送具备网络系统思维的工程师。"));
